...
def gen_binary_off_list(name_in):
	...
	names = [name_in]
	for name in names:
		print ("___________________________________________", name, "___________________________________________")	
		try:
			...
			print (pose.pdb_info())
			dist_dict = getAllDistanceMatrix(pose)
			bb_list = getAllbbSASA(pose)
			hb_dict = getAllHBonds(pose)
			xarray = np.zeros((1,293))
			...
			if xarray.shape[0] > 0:
				xarray.tofile(name + "binaryX", sep ="")
...
names_raw = open("misshapenFiles.txt", "r")
names = names_raw.read().splitlines()


failures = open("./failedResidues2.txt", "w")
